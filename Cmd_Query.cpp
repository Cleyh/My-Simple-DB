            ...
            */
            else if (_op == "-C")
            {
                if (_tokens.size() == 6)  // 查某列
                {
                    ...
                    vector<string> result = _db.query_scol(_db_name, _ls_name, col);
                    json j = vector_to_json({ result });
                    _return = j.dump(-1);
                }
